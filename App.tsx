...
export default function App() {
  ...
  async function checkApiConnection() {
    ...
    try {
      ...
      if (response && response.text) { 
        ...
        addNotification("Vibe connection established! ✨", "success");
        return true; 
      }
      throw new Error("Bad key");
    } catch (error: any) {
      setApiStatus('error');
      showToast("API Connection failed. Please select a valid key.", "error");
      return false;
    }
  }
  ...
  const handleSelectApiKey = async () => {
    if (window.aistudio?.openSelectKey) {
      ...
      showToast("Waiting for key selection...", "info");
  ...
  const handleDeleteSession = (id: string) => {
    if (confirm("End this vibe session?")) {
      ...
      showToast("Vibe session ended.", "info");
    }
  };
  ...
  async function handleSendToAI(text: string, fileData?: { data: string, mimeType: string, fileName: string }) {
    ...
    if (apiStatus !== 'connected') {
      ...
      if (!isOk) {
        showToast("Vibe connection lost. Please check your API key in Profile.", "error");
        return;
      }
    }
    ...
    try {
      ...
      showToast("Vibe interrupted. Check your network or API quota! ⚡", "error");
    } finally { 
      setIsLoading(false); 
    }
  }
  ...
  if (isNewUser) {
    ...
    const DiscoveryStep = ({ title, options, current, onSelect, multi = false, onNext }: any) => (
      <div className="space-y-8 animate-slide-in-right">
        ...
        <div className="grid grid-cols-1 xs:grid-cols-2 gap-3 max-h-[45vh] overflow-y-auto pr-2 custom-scrollbar">
          {options.map((opt: any) => {
          ...
          {onboardingStep === 1 ? (
            <div className="space-y-10 animate-slide-up">
              ...
              <div className="space-y-3">
                ...
                <p className="text-zinc-500 font-medium text-sm md:text-base">Meet Mr. Cute. Your AI best friend.</p>
              </div>
              ...
              <button onClick={() => { if (!validateEmail(credentials.email)) { showToast("Enter a valid email! 📧", "error"); return; } setOnboardingStep(1.5); }} className="w-full bg-blue-600 hover:bg-blue-500 text-white py-4 md:py-5 rounded-2xl font-black text-lg shadow-xl transition-all active:scale-95">Proceed to Vibe Check</button>
            </div>
          ) : onboardingStep === 1.5 ? (
            <div className="space-y-10 animate-slide-in-right">
              ...
              <div className="space-y-4">
                ...
                <h2 className="text-2xl md:text-3xl font-black italic text-zinc-900 dark:text-white tracking-tighter">Connection First</h2>
                <p className="text-zinc-500 text-sm md:text-base font-medium px-4">Mr. Cute needs a key to start your journey. Please select a valid Gemini API key to continue.</p>
              </div>
              <button onClick={async () => { await handleSelectApiKey(); setOnboardingStep(2); }} className="w-full bg-zinc-900 dark:bg-white text-white dark:text-black py-4 md:py-5 rounded-2xl font-black text-lg shadow-xl transition-all active:scale-95 flex items-center justify-center gap-3">
                <Activity size={20} /> Select Vibe Key
              </button>
              <p className="text-[10px] uppercase font-black tracking-widest text-zinc-400">Status: {apiStatus.toUpperCase()}</p>
            </div>
          ) : onboardingStep === 2 ? (
            <div className="space-y-8 md:space-y-10 animate-slide-in-right">
              ...
              <h2 className="text-2xl md:text-3xl font-black italic text-zinc-900 dark:text-white tracking-tighter">Visual Identity</h2>
              <div className="flex justify-center flex-wrap gap-3 md:gap-4">
                {AVATARS.map((url) => (
                  <button key={url} onClick={() => setTempProfile({...tempProfile, avatarUrl: url})} className={`w-14 h-14 md:w-20 md:h-20 rounded-[1.8rem] overflow-hidden transition-all shadow-lg ${tempProfile.avatarUrl === url ? 'ring-4 ring-blue-500 scale-110' : 'opacity-40 hover:opacity-100'}`}>
                    <img src={url} className="w-full h-full" alt="Vibe Visual" />
                  </button>
                ))}
              </div>
            ...
            <DiscoveryStep title="Favorite Genre" options={DISCOVERY_DATA.movies} current={tempProfile.movieGenre} onSelect={(v: string) => setTempProfile({...tempProfile, movieGenre: v})} onNext={() => setOnboardingStep(6)} />
          ) : onboardingStep === 6 ? (
            ...
            <div className="space-y-10 animate-slide-in-right">
              ...
              <button onClick={() => setUser(tempProfile as User)} className="w-full bg-blue-600 hover:bg-blue-500 text-white py-4 md:py-5 rounded-2xl font-black text-lg shadow-xl transition-all active:scale-95">Catch My Vibe!</button>
            </div>
          )}
        </div>
      </div>
    ...
    <div className="flex h-screen w-full font-sans overflow-hidden bg-zinc-50 dark:bg-[#050505] transition-colors duration-500 relative">
          ...
          <div className="flex-1 flex flex-col items-center justify-center gap-8 md:gap-10 text-center w-full">
            ...
            <div className="space-y-3 md:space-y-4">
              ...
              <p className="text-blue-600 dark:text-blue-400 font-bold uppercase tracking-widest text-[10px] md:text-xs animate-pulse">
                {isLive ? "Connection Live" : "Tuning In..."}
              </p>
            </div>
      ...
      <div className="flex-1 flex flex-col relative h-full overflow-hidden w-full">
        ...
        <header className="px-4 md:px-6 py-4 border-b border-zinc-100 dark:border-white/5 flex items-center justify-between bg-white/60 dark:bg-black/40 backdrop-blur-2xl sticky top-0 z-[300] w-full">
          ...
          <div className="flex items-center gap-2 md:gap-4">
            <div className="relative">
              ...
              {isNotifOpen && (
                <div className="fixed sm:absolute top-full right-4 sm:right-0 mt-4 w-[calc(100vw-32px)] sm:w-80 bg-white dark:bg-zinc-900 border border-zinc-100 dark:border-white/10 rounded-[2.5rem] shadow-[0_20px_60px_-15px_rgba(0,0,0,0.3)] p-6 z-[1000] animate-vibe-in overflow-hidden">
                  <div className="flex items-center justify-between mb-5">
                    <h3 className="text-[11px] font-black uppercase tracking-[0.2em] text-zinc-900 dark:text-white">Soul Activity</h3>
                    <button onClick={() => setNotifications([])} className="text-[10px] font-black text-rose-500 uppercase hover:bg-rose-500/10 px-2 py-1 rounded-lg transition-colors">Clear</button>
                  </div>
                  <div className="space-y-4 max-h-[50vh] overflow-y-auto custom-scrollbar pr-1">
                    {notifications.length === 0 ? (
                      <div className="py-10 text-center space-y-2">
                        <Activity size={24} className="mx-auto text-zinc-200 dark:text-zinc-700" />
                        <p className="text-[10px] text-zinc-400 font-bold uppercase tracking-widest">No recent vibes.</p>
                      </div>
                    ) : notifications.map(n => (
                      <div key={n.id} className="flex gap-4 items-start border-b border-zinc-100 dark:border-white/5 pb-4 last:border-0 group">
                        <div className={`w-2.5 h-2.5 rounded-full mt-1.5 shrink-0 transition-transform group-hover:scale-125 ${n.type === 'error' ? 'bg-rose-500' : n.type === 'success' ? 'bg-green-500' : 'bg-blue-500'}`} />
                        <div className="flex-1 space-y-1">
                          <p className="text-[12px] font-bold text-zinc-900 dark:text-zinc-200 leading-snug">{n.text}</p>
                          <p className="text-[9px] text-zinc-400 uppercase font-black tracking-tight">{new Date(n.time).toLocaleTimeString([], {hour:'2-digit', minute:'2-digit'})}</p>
                        </div>
                      </div>
                    ))}
                  </div>
          ...
          <div className="max-w-3xl mx-auto flex flex-col gap-8 md:gap-12 pb-32">
            {messages.length === 0 && !isLoading ? (
              <div className="min-h-[60vh] flex flex-col items-center justify-center text-center space-y-6 md:space-y-8 animate-vibe-in">
                ...
                <div className="space-y-3">
                  <h2 className="text-3xl md:text-4xl font-black text-zinc-900 dark:text-white italic tracking-tighter uppercase leading-none">Ready to Vibe?</h2>
                  <p className="text-[13px] md:text-sm font-medium text-zinc-500 max-w-xs mx-auto">Mr. Cute is tuned into your soul. Send him a message or tap the mic!</p>
                </div>
              </div>
              ...
              <div key={msg.id} className={`flex w-full group ${msg.role === 'user' ? 'justify-end' : 'justify-start'} animate-vibe-in`} style={{ animationDelay: `${idx * 0.05}s` }}>
                <div className={`flex items-end gap-2 md:gap-3 max-w-[92%] sm:max-w-[85%] md:max-w-[75%] ${msg.role === 'user' ? 'flex-row-reverse' : 'flex-row'}`}>
                  {msg.role === 'model' && (
                  ...
                  <div className={`flex flex-col gap-1.5 md:gap-2 ${msg.role === 'user' ? 'items-end' : 'items-start'}`}>
                    <div className={`px-5 py-3.5 md:px-6 md:py-4 rounded-[1.8rem] md:rounded-[2.2rem] shadow-xl text-[14px] md:text-[1rem] leading-relaxed font-bold transition-all relative
                      ${msg.role === 'user' 
                      ...
                      {msg.image && <div className="mb-3 md:mb-4 rounded-xl md:rounded-2xl overflow-hidden shadow-2xl border border-zinc-100 dark:border-white/10"><img src={msg.image} alt="Vibe" className="w-full h-auto max-h-[300px] md:max-h-[400px] object-cover" /></div>}
                      <p className="whitespace-pre-wrap">{msg.text}</p>
                    </div>
                    
                    {/* Interaction Bar */}
                    <div className={`flex items-center gap-3 md:gap-4 px-2 ${msg.role === 'user' ? 'flex-row-reverse' : 'flex-row'}`}>
                       ...
                       <div className="flex items-center gap-1.5 opacity-0 group-hover:opacity-100 transition-all duration-300 pointer-events-none group-hover:pointer-events-auto">
                          <button onClick={() => handleReaction(msg.id, 'like')} title="Like" className={`p-1.5 rounded-full transition-all ${msg.reaction === 'like' ? 'text-blue-500 bg-blue-500/10' : 'text-zinc-400 hover:text-blue-500 hover:bg-zinc-100 dark:hover:bg-zinc-800'}`}>
                            <ThumbsUp size={13} fill={msg.reaction === 'like' ? 'currentColor' : 'none'} />
                          </button>
        ...
        <footer className="px-4 py-6 md:py-8 pointer-events-none absolute bottom-0 left-0 right-0 z-[200]">
          <div className="max-w-2xl mx-auto flex items-center gap-3 pointer-events-auto w-full">
            <div className="flex-1 bg-white/95 dark:bg-zinc-900/95 backdrop-blur-3xl flex items-center rounded-[2.5rem] px-4 md:px-6 py-1 border-2 border-zinc-200 dark:border-white/10 focus-within:border-blue-500 transition-all shadow-[0_15px_40px_-10px_rgba(0,0,0,0.15)]">
      ...
      {isProfileModalOpen && (
        <div className="fixed inset-0 z-[6000] flex items-center justify-center p-4 md:p-6">
          ...
          <div className="relative w-full max-w-xl bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-white/10 rounded-[2.5rem] md:rounded-[3rem] p-6 md:p-10 shadow-3xl animate-vibe-in max-h-[95vh] overflow-y-auto custom-scrollbar">
            ...
            <div className="space-y-10 md:space-y-12">
              <div className="flex flex-col items-center gap-6 text-center">
                <div className="relative group">
                  <img src={user?.avatarUrl} className="w-24 h-24 md:w-32 md:h-32 rounded-[2rem] md:rounded-[2.5rem] shadow-2xl border-4 border-white dark:border-zinc-800 transition-transform group-hover:scale-105" alt="Avatar" />
                ...
                <div className="w-full space-y-4 md:space-y-5 text-left">
                  <div className="space-y-2 px-1">
                    ...
                    <div className="flex gap-2">
                      ...
                      />
                      <button onClick={handleUpdateUser} className="bg-blue-600 text-white px-5 rounded-2xl hover:bg-blue-500 transition-all active:scale-95 shadow-lg shadow-blue-500/20">
                        <Check size={20} strokeWidth={3} />
                      </button>
                    </div>
                  </div>

                  <div className="p-5 md:p-6 bg-zinc-50 dark:bg-zinc-800/40 rounded-[2rem] border border-zinc-100 dark:border-white/5 space-y-4">
                    <div className="flex items-center justify-between">
                      <label className="text-[10px] md:text-[11px] font-black uppercase tracking-[0.2em] text-zinc-400 dark:text-zinc-500 block">System Connection</label>
                      <div className={`w-3 h-3 rounded-full ${apiStatus === 'connected' ? 'bg-green-500' : apiStatus === 'checking' ? 'bg-amber-500' : 'bg-rose-500 animate-pulse'}`} />
                    </div>
                    <button onClick={handleSelectApiKey} className="w-full flex items-center justify-center gap-3 bg-zinc-900 dark:bg-white text-white dark:text-black py-4 rounded-2xl font-black text-[10px] md:text-xs uppercase tracking-[0.1em] shadow-xl hover:scale-[1.02] active:scale-95 transition-all">
                      <RefreshCw size={14} className={apiStatus === 'checking' ? 'animate-spin' : ''} />
                      Change Gemini Vibe Key
                    </button>
                    <div className="flex items-center justify-between px-1">
                      <p className={`text-[9px] font-black uppercase tracking-widest ${apiStatus === 'error' ? 'text-rose-500' : 'text-zinc-400'}`}>Status: {apiStatus.toUpperCase()}</p>
                      <a href="https://ai.google.dev/gemini-api/docs/billing" target="_blank" className="text-[9px] font-black text-blue-500 hover:underline uppercase tracking-widest">Billing Docs</a>
                    </div>
                  </div>
                  
                  <div className="flex justify-center flex-wrap gap-2 pt-1 px-1">
                    {[user?.movieGenre, user?.musicGenre, user?.educationLevel, `Age ${user?.age}`].map((tag, i) => (
                      <span key={i} className="px-4 py-2 bg-zinc-100 dark:bg-zinc-800 rounded-full text-[9px] font-black uppercase tracking-widest text-zinc-600 dark:text-zinc-400 border border-zinc-200 dark:border-white/5">{tag}</span>
                    ))}
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-6">
                <label className="text-[10px] md:text-[11px] font-black uppercase tracking-[0.2em] text-zinc-400 dark:text-zinc-500 px-1">Soul Essence (Personality)</label>
                <div className="grid grid-cols-2 gap-3 md:gap-4 max-h-[30vh] md:max-h-none overflow-y-auto custom-scrollbar">
                  {Object.values(PERSONALITIES).map(p => (
                    <button key={p.id} onClick={() => { setSettings({...settings, personalityId: p.id, voiceName: p.voiceName}); showToast(`${p.name} Essence Loaded! ✨`, "success"); }} className={`flex items-center gap-3 md:gap-4 p-4 md:p-5 rounded-[2rem] border-2 transition-all shadow-sm ${settings.personalityId === p.id ? 'bg-blue-600 border-blue-500 text-white shadow-xl scale-[1.03]' : 'bg-zinc-100 dark:bg-zinc-800 border-transparent text-zinc-900 dark:text-white hover:bg-zinc-200 dark:hover:bg-zinc-800'}`}>
              ...
              <div className="pt-6">
                <button onClick={handleLogOut} className="w-full py-5 md:py-6 text-[11px] md:text-[12px] text-rose-500 font-black uppercase tracking-[0.4em] hover:bg-rose-500/10 rounded-3xl transition-all border-2 border-rose-500/20 shadow-lg shadow-rose-500/5">End Journey</button>
              </div>
            </div>
          </div>
        </div>
