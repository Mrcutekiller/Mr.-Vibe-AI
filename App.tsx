...
export default function App() {
  ...
  // Neural Pulse Logic: Check history for reminders on app mount
  useEffect(() => {
  ...
  const checkForNeuralPulse = async () => {
    ...
    try {
      ...
      // Look at the last session's text context
      const lastSession = sessions[0];
      const context = lastSession.messages.slice(-10).map(m => m.text).join('\n');
      
      const response = await ai.models.generateContent({
        model: 'gemini-3-flash-preview',
        contents: [{ text: `CONTEXT HISTORY: ${context}\n\nTASK: You are Mr. Cute. Scan the context for any upcoming events (test, date, meeting) or things the user mentioned they were going to do. If found, generate a SHORT slang-heavy follow-up question (max 12 words). If nothing relevant is found, return exactly 'none'.` }]
      });
      ...
      if (pulseText && pulseText.toLowerCase() !== 'none') {
        showToast(pulseText, 'pulse', () => {
          ...
          handleSendToAI(`Yo bestie, you asked: "${pulseText}". Let's link on that.`, true);
          setToast(null);
        });
      }
    } catch (e) {
  ...
  const generateInitialGreeting = async (sessionId: string, personalityId: PersonalityId) => {
    ...
    try {
      ...
      const prompt = `ACT AS Mr. Cute with the ${persona.name} persona. 
      The user just opened the app. 
      Greet the user warmly by their name ${user?.userName || tempProfile.userName} (Gender context: ${user?.gender || tempProfile.gender}). 
      Ask them how their day was or drop a vibe-sync greeting in your unique SAVAGE or CHARISMATIC style. 
      Keep it short, WhatsApp style. 
      IMPORTANT: You ARE allowed to use your name 'Mr. Cute' ONLY in this first message.`;
  ...
  const handleNewChat = useCallback((autoGreet = true) => {
    ...
    return newId;
  }, [settings.personalityId, user, hasLicense, tempProfile]);
  ...
  const generateNeuralQuiz = async (specificMessageId?: string) => {
    ...
    try {
      ...
      if (e.message?.includes("Requested entity was not found")) {
        setHasLicense(false);
        showToast("License expired. Re-verify.", "error");
      } else {
  ...
  async function handleSendToAI(text: string, isAutoGreet = false) {
    if (!hasLicense) {
      showToast("LICENSE KEY required for neural link.", "error");
    ...
    try {
      ...
      if (currentFile?.type === 'application/pdf' || currentFile?.type.startsWith('image/')) {
        parts.push({
          inlineData: {
      ...
      const aiMessage: Message = { id: `ai-${Date.now()}`, role: 'model', text: response.text || '...', timestamp: Date.now(), isNote: selectedVoiceMode === 'note' };
      ...
      if (e.message?.includes("Requested entity was not found")) {
        setHasLicense(false);
        showToast("License verification required.", "error");
      } else {
  ...
  const handleVoiceButtonClick = () => {
    if (!hasLicense) {
      showToast("LICENSE KEY required for voice.", "error");
      handleOpenLicenseKey();
      return;
    }
    if (isLive) {
  ...
  return (
    <div className={`flex flex-col h-full w-full transition-colors duration-500 ${theme === 'dark' ? 'bg-[#050505] text-white' : 'bg-zinc-50 text-black'} relative overflow-hidden ios-safe-top ios-safe-bottom font-sans`}>
      ...
      <header className={`h-20 px-8 flex items-center justify-between border-b ${theme === 'dark' ? 'border-white/5 bg-black/40' : 'border-black/5 bg-white/40'} backdrop-blur-3xl z-50`}>
        ...
        <div className="flex flex-col items-center">
          ...
          <span className={`text-[9px] font-black flex items-center gap-1.5 uppercase tracking-widest mt-1 ${theme === 'dark' ? 'text-zinc-500' : 'text-zinc-400'}`}>
             <span className={`w-1.5 h-1.5 rounded-full ${isSyncingMemories ? 'bg-blue-500 animate-ping' : hasLicense ? 'bg-emerald-500 animate-pulse' : 'bg-rose-500'}`} /> {isSyncingMemories ? 'Neural Syncing...' : hasLicense ? 'Neural Sync Active' : 'License Required'}
          </span>
        </div>
      ...
      <main ref={mainContentRef} className="flex-1 overflow-y-auto px-6 py-10 space-y-10 custom-scrollbar scroll-smooth">
        {messages.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full text-center space-y-12 animate-fade-in">
            ...
            <div className="space-y-4 max-w-xs">
              <h3 className="text-sm font-black uppercase tracking-[0.5em] text-blue-500">Awaiting Signal</h3>
              <p className="text-[12px] font-bold italic text-zinc-500 uppercase tracking-widest leading-relaxed">Choose your mode below and let's link frequencies.</p>
            </div>
          </div>
        ) : (
          messages.map((msg) => (
            <div key={msg.id} className={`flex flex-col ${msg.role === 'user' ? 'items-end' : 'items-start'} animate-vibe-in group`}>
              {msg.file && (
                <div className={`mb-2 max-w-[80%] rounded-[28px] overflow-hidden border shadow-xl ${theme === 'dark' ? 'border-white/10' : 'border-black/5'}`}>
                  {msg.file.type.startsWith('image/') ? (
                    <img src={msg.file.data} alt="Vibe" className="w-full h-auto object-cover max-h-[300px]" />
                  ) : (
                    <div className="flex items-center gap-4 p-5 bg-white/5">
                      <div className="p-3 bg-blue-500/10 rounded-2xl text-blue-500">
                        {msg.file.type === 'application/pdf' ? <FileText size={24} /> : <FileIcon size={24} />}
                      </div>
              ...
              <div className="relative flex items-center gap-2 max-w-[90%]">
                <div className={`px-6 py-4 rounded-[28px] text-sm font-semibold shadow-xl border transition-all duration-500 ${
                  msg.role === 'user' 
                    ? 'bg-gradient-to-br from-blue-600 to-blue-700 text-white border-blue-500/20 rounded-tr-sm' 
                    : theme === 'dark' 
                      ? 'bg-[#1a1a1a] text-zinc-100 border-white/5 rounded-tl-sm' 
                      : 'bg-white text-zinc-900 border-black/5 rounded-tl-sm'
                }`}>
                  {msg.isNote && (
                    <div className="flex items-center gap-1.5 mb-2 text-[9px] font-black uppercase tracking-widest text-blue-400">
                      <StickyNote size={10} /> Saved Frequency
                    </div>
                  )}
                ...
                {msg.role === 'model' && (
                  <div className="flex flex-col gap-2">
                    <button 
                      ...
                      className={`p-3 rounded-2xl transition-all flex items-center gap-2 ${msg.isPinned ? 'bg-blue-600 text-white scale-110 shadow-lg shadow-blue-600/30' : 'text-zinc-500 hover:text-blue-500 opacity-0 group-hover:opacity-100'}`}
                    >
                      <Pin size={16} fill={msg.isPinned ? 'currentColor' : 'none'} />
                      {msg.isNote && !msg.isPinned && <span className="text-[8px] font-black uppercase tracking-widest hidden group-hover:inline">Save to Brain</span>}
                    </button>
                  </div>
                )}
              </div>
        ...
        {liveTranscript.length > 0 && (
          <div className="flex flex-col gap-4">
            {liveTranscript.map((t, idx) => (
              <div key={idx} className={`flex flex-col ${t.isModel ? 'items-start' : 'items-end'} animate-fade-in`}>
                <div className={`px-6 py-4 rounded-[28px] text-sm font-semibold opacity-60 italic border ${
                  t.isModel 
                    ? theme === 'dark' ? 'bg-[#1a1a1a]/50 border-white/5 rounded-tl-sm' : 'bg-white/50 border-black/5 rounded-tl-sm'
                    : 'bg-blue-600/30 text-white border-blue-500/10 rounded-tr-sm'
                }`}>
                  {t.text}
                </div>
              </div>
        ...
        {isLoading && (
          <div className="flex items-start">
            <div className={`px-6 py-4 rounded-[28px] rounded-tl-sm border ${theme === 'dark' ? 'bg-[#1a1a1a] border-white/5' : 'bg-white border-black/5'}`}>
      ...
      <footer className={`p-6 bg-gradient-to-t ${theme === 'dark' ? 'from-black via-black/95' : 'from-zinc-50 via-zinc-50/95'} to-transparent z-40`}>
        {selectedFile && (
          <div className="max-w-2xl mx-auto mb-4 animate-slide-up">
            <div className={`relative inline-flex items-center gap-4 p-4 rounded-[28px] border shadow-2xl ${theme === 'dark' ? 'bg-[#1a1a1a] border-white/10' : 'bg-white border-black/10'}`}>
              <div className="w-10 h-10 rounded-xl overflow-hidden border border-white/10">
                {selectedFile.type.startsWith('image/') ? <img src={selectedFile.data} alt="Preview" className="w-full h-full object-cover" /> : <div className="w-full h-full bg-blue-500/10 flex items-center justify-center text-blue-500">
                  {selectedFile.type === 'application/pdf' ? <FileText size={18} /> : <FileIcon size={18} />}
                </div>}
              </div>
              <div className="text-[10px] font-black truncate max-w-[150px]">{selectedFile.name}</div>
              <button onClick={() => setSelectedFile(null)} className="p-2 hover:bg-white/10 rounded-xl text-zinc-500"><X size={14} /></button>
            </div>
          </div>
        )}
        <div className={`max-w-4xl mx-auto flex items-center gap-3 p-2.5 border rounded-[36px] shadow-2xl backdrop-blur-[50px] transition-all duration-500 ${theme === 'dark' ? 'bg-[#111111]/90 border-white/10 ring-1 ring-white/5' : 'bg-white border-black/10 ring-1 ring-black/5'}`}>
          ...
          <input 
            type="text" 
            placeholder={hasLicense ? "Link frequencies..." : "Activate LICENSE KEY to chat..."}
            value={inputText} 
            onChange={e => setInputText(e.target.value)} 
            onKeyDown={e => e.key === 'Enter' && handleSendToAI(inputText)} 
            className="flex-1 bg-transparent py-4 px-3 font-bold text-sm outline-none text-white placeholder-zinc-700 disabled:opacity-50"
          />
          <div className="flex items-center gap-2.5 pr-2.5">
             <button onClick={handleVoiceButtonClick} className={`p-4 rounded-full transition-all active:scale-90 ${isLive ? 'bg-rose-600 text-white animate-pulse' : 'bg-white/5 text-zinc-500'}`}>{isLive ? <MicOff size={24}/> : <Mic size={24}/>}</button>
             <button onClick={() => handleSendToAI(inputText)} className={`p-4 rounded-full transition-all duration-500 active:scale-95 ${(inputText.trim() || selectedFile) ? 'bg-blue-600 text-white shadow-lg shadow-blue-600/30' : 'bg-zinc-500/10 text-zinc-500/20'}`}><Send size={24}/></button>
          </div>
        </div>
      </footer>
      ...
      {isVoiceModeModalOpen && (
        <div className="fixed inset-0 z-[11000] flex items-center justify-center p-8 animate-fade-in">
          <div className="absolute inset-0 bg-black/90 backdrop-blur-3xl" onClick={() => setIsVoiceModeModalOpen(false)} />
          <div className={`relative w-full max-w-sm rounded-[56px] p-12 space-y-12 border shadow-2xl animate-scale-in ${theme === 'dark' ? 'bg-[#0a0a0a] border-white/10' : 'bg-white border-black/10'}`}>
            <div className="text-center space-y-4">
              <div className="w-16 h-16 bg-blue-600/10 rounded-3xl flex items-center justify-center mx-auto text-blue-500"><Mic size={32}/></div>
              <h2 className="text-xl font-black uppercase italic tracking-tighter">Voice Frequency</h2>
              <p className="text-[10px] font-black text-zinc-600 uppercase tracking-widest leading-relaxed">Choose your link protocol, bestie.</p>
            </div>
            
            <div className="space-y-4">
              <button onClick={() => startVoiceMode('chat')} className="w-full py-6 bg-blue-600 text-white rounded-[32px] font-black uppercase text-xs tracking-[0.3em] flex items-center justify-center gap-4 active:scale-95 transition-all shadow-xl shadow-blue-600/20">
                <MessageCircle size={20}/> Voice Chat
              </button>
              <button onClick={() => startVoiceMode('note')} className="w-full py-6 bg-zinc-800 text-white rounded-[32px] font-black uppercase text-xs tracking-[0.3em] flex items-center justify-center gap-4 active:scale-95 transition-all shadow-xl">
                <StickyNote size={20}/> Note Taker
              </button>
            </div>
            
            <button onClick={() => setIsVoiceModeModalOpen(false)} className="w-full py-4 text-[10px] font-black uppercase tracking-widest text-zinc-600">Cancel</button>
          </div>
        </div>
      )}
      ...
      {isProfileModalOpen && (
        <div className="fixed inset-0 z-[8000] flex items-center justify-center p-6">
           ...
           <div className={`relative w-full max-w-xl rounded-[60px] p-12 space-y-10 animate-scale-in border shadow-2xl ${theme === 'dark' ? 'bg-[#0a0a0a] border-white/10' : 'bg-white border-black/10'} max-h-[90vh] overflow-y-auto custom-scrollbar`}>
              ...
              <div className="space-y-8">
                 <div className="p-8 rounded-[40px] bg-blue-600/5 border border-blue-500/20 space-y-6">
                    <div className="flex items-center justify-between">
                       <label className="text-[10px] font-black text-blue-500 uppercase tracking-widest">Neural License</label>
                       <span className={`text-[9px] font-black px-3 py-1 rounded-full uppercase ${hasLicense ? 'bg-emerald-500/20 text-emerald-400' : 'bg-rose-500/20 text-rose-400'}`}>
                          {hasLicense ? 'Verified' : 'Required'}
                       </span>
                    </div>
                    <button onClick={handleOpenLicenseKey} className="w-full py-4 bg-white/5 border border-white/10 rounded-[24px] font-black text-[10px] uppercase tracking-widest flex items-center justify-center gap-3 hover:bg-white/10 transition-all">
                       <Key size={16} /> Update License Key
                    </button>
                 </div>
      ...
      {isNewUser && (
        <div className="fixed inset-0 z-[9000] bg-black flex items-center justify-center p-8 overflow-y-auto custom-scrollbar">
          <div className="w-full max-w-2xl bg-[#080808] rounded-[64px] p-16 text-center border border-white/5 animate-scale-in my-auto">
             ...
             <h1 className="text-4xl font-black mb-16 uppercase italic tracking-tighter text-white">Neural Protocol</h1>
             
             <div className="space-y-12 text-left mb-16">
               <div className="p-10 rounded-[48px] bg-blue-600/5 border-2 border-blue-500/20 space-y-6">
                 <div className="flex items-center justify-between mb-4">
                   <h3 className="text-lg font-black uppercase italic tracking-tighter flex items-center gap-3"><Key size={22} className="text-blue-500" /> Neural License</h3>
                   <span className={`text-[10px] font-black px-4 py-1.5 rounded-full uppercase tracking-widest ${hasLicense ? 'bg-emerald-500 text-white' : 'bg-rose-500/20 text-rose-400'}`}>
                      {hasLicense ? 'Verified' : 'Missing'}
                   </span>
                 </div>
                 <p className="text-[11px] font-bold text-zinc-500 leading-relaxed italic mb-4">Activation of your License Key is required to establish a neural frequency link with Mr. Cute.</p>
                 <button onClick={handleOpenLicenseKey} className={`w-full py-6 rounded-[32px] font-black text-xs uppercase tracking-[0.3em] flex items-center justify-center gap-4 transition-all shadow-xl active:scale-95 ${hasLicense ? 'bg-emerald-500 text-white' : 'bg-blue-600 text-white shadow-blue-600/20'}`}>
                    {hasLicense ? <CheckCircle2 size={20} /> : <Zap size={20} />} {hasLicense ? 'License Active' : 'Activate License Key'}
                 </button>
                 <div className="text-center">
                   <a href="https://ai.google.dev/gemini-api/docs/billing" target="_blank" className="text-[10px] font-black text-zinc-600 uppercase tracking-widest hover:text-blue-500 flex items-center justify-center gap-2">
                     License Documentation <ExternalLink size={12}/>
                   </a>
                 </div>
               </div>
             ...
             <button onClick={() => { 
                if (!hasLicense) { showToast("License activation required.", "error"); return; }
             ...
             }} className={`w-full py-7 rounded-[40px] font-black text-xl uppercase tracking-[0.3em] active:scale-95 transition-all shadow-2xl ${hasLicense ? 'bg-blue-600 text-white' : 'bg-zinc-800 text-zinc-600 opacity-50 cursor-not-allowed'}`}>Link Persona</button>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-[10000] bg-black/98 backdrop-blur-3xl flex flex-col p-8 md:p-16 overflow-y-auto custom-scrollbar animate-fade-in">
          <div className="w-full max-w-3xl mx-auto space-y-12 pb-20">
            ...
            {!isQuizSubmitted ? (
              ...
              }} className="w-full py-8 bg-blue-600 text-white rounded-[40px] font-black text-xl uppercase tracking-[0.3em] active:scale-95 transition-all shadow-2xl">Submit Assessment</button>
            ) : (
              <div className="flex flex-col items-center gap-10 animate-slide-up">
                ...
                <button onClick={() => setActiveQuiz(null)} className="px-12 py-6 bg-white text-black rounded-[40px] font-black uppercase text-xs tracking-widest active:scale-95 transition-all shadow-xl">Finish Assessment</button>
              </div>
            )}
          </div>
        </div>
      ...
      <div className={`fixed inset-y-0 left-0 z-[10000] w-80 transition-transform duration-700 transform ${isHistoryOpen ? 'translate-x-0' : '-translate-x-full'} shadow-[0_0_100px_rgba(0,0,0,0.5)] border-r ${theme === 'dark' ? 'bg-[#0a0a0a] border-white/5' : 'bg-white border-black/5'}`}>
         <div className="flex flex-col h-full">
            <div className={`p-10 flex items-center justify-between border-b ${theme === 'dark' ? 'border-white/5' : 'border-black/5'}`}>
               <h2 className="font-black uppercase tracking-[0.4em] text-[10px] flex items-center gap-3 text-zinc-600"><HistoryIcon size={18}/> Vibe Archive</h2>
               ...
               {pinnedMessages.length === 0 ? (
                 ...
                 >
                    <div className="flex items-center justify-between mb-2">
                      <span className={`text-[8px] font-black uppercase tracking-widest px-2 py-0.5 rounded-full ${m.isNote ? 'bg-blue-500/20 text-blue-400' : 'bg-zinc-500/20 text-zinc-400'}`}>
                        {m.isNote ? 'Study Brain' : 'Vibe Log'}
                      </span>
                      <span className="text-[8px] font-bold text-zinc-600">{new Date(m.timestamp).toLocaleDateString()}</span>
                    </div>
